...
| ファイル | 対象 |
|---|---|
| `scripts/test_song_stats.py` | 曲ごとの指標。在籍区間の境界、同順位タイの日付、空の結果、表示書式 |
| `scripts/test_checks.py` | 件数照合と取得ログ。`failed` と `missing` の区別 |

提案の書式検査は `python scripts/check_proposals.py`。テストとは別のジョブで走る。
...
取得の直後に整合性チェックが走り、落ちればワークフローは失敗（赤）になる。
ただし取得できた当日分を捨てないよう、コミットと push はチェックが落ちた場合でも実行される。
過去日は取得し直せないので、データの確保を検査より優先している。

実行の順序は 取得 → track_id の安定性 → 件数照合 → 取得状況の表示 → コミットと push。

### ワークフローは2つに分かれている

通知の意味を1対1に保つため、データとそれ以外を同じワークフローに混ぜていない。
赤いメールを見たときに原因が判別できないと、赤に慣れて本当の取得失敗を見逃すため。

| ワークフロー | 赤い時の意味 | 実行 |
|---|---|---|
| `daily-chart` | **その日のデータが欠けている** | 1日3回 + 手動 |
| `checks` | コードのテストか提案の書式が落ちた。データは無事 | 週1（日曜） + 手動 + `scripts/` `proposals/` の変更時 |

`checks` は「テスト」と「提案の書式検査」の2ジョブに分かれており、一覧の時点でどちらが落ちたか分かる。
...
```bash
python scripts/build_site.py     # site/ に出力
```

標準ライブラリのみで生成する。`site/` は `data/` からいつでも作り直せるため追跡しない。
ブラウザで `site/index.html` を直接開けば確認できる（外部ファイルを読みに行かない作りにしてある）。

一覧が主役で、並べ替え（順位順・新入り順・古株順・ロングヒット）と月別絞り込みは
**組み合わせのぶんだけページを事前生成**する。JavaScript なしで動き、どのページも単体で開ける。
ページ数は（月数 + 1）× 4 + 検索。

検索だけは結果が動的なので、索引をページに埋め込んでブラウザ側で絞り込む。
検索結果ページは `noindex`。
